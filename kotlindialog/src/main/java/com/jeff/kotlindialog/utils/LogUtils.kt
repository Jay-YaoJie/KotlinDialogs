...
object  LogUtils {
    ...
    fun i(tag: String, msg: String) = DialogSettings.LOG_DEBUG.debugLog(tag, "${tag}==---==${msg}", Log.INFO)
    fun w(tag: String, msg: String) = DialogSettings.LOG_DEBUG.debugLog(tag, "${tag}==---==${msg}", Log.WARN)
    fun e(tag: String, msg: String) = DialogSettings.LOG_DEBUG.debugLog(tag, "${tag}==---==${msg}", Log.ERROR)
    //打印或保存到日志
